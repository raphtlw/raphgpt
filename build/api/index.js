...
bot.command("learn", async (ctx) => {
    const fromConv = ctx.session.conversations[ctx.chat.id][ctx.session.conversations[ctx.chat.id]
        .map((conv) => conv.role === "user")
        .lastIndexOf(true)];
    await ctx.api.sendMessage(ctx.chat.id, `How should I respond instead to "${fromConv.content}"?`);
    ctx.session.learn = {
        input: fromConv.content,
    };
});
bot.on("message", async (ctx) => {
    ...
    console.log(JSON.stringify(ctx.session.conversations[ctx.chat.id], undefined, 4));
    ...
    let sentResponseMessage = null;
    let completionLastChunk = null;
    ...
    for await (const chunk of completion) {
        ...
        if (!sentResponseMessage) {
            ...
            if (sentResponse.text !== sentResponse.prev) {
                try {
                    await ctx.api.editMessageText(ctx.chat.id, sentResponseMessage.message_id, sentResponse.text);
                }
                catch (e) {
    ...
    if (completionLastChunk) {
        const generation = await fetch(`https://openrouter.ai/api/v1/generation?id=${completionLastChunk.id}`, {
            headers: {
